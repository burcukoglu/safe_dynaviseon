...
class KITTI_Dataset(Dataset):
    ...
    def __init__(self, directory='./datasets/kitti',
        ...
        # if self._mask is not None:
        #         # self.data = self.data*self._mask
        #         self.data = data*self._mask
        ...
        # end=time.time()
        # print('load to device', end-first)
    ...
    def save_equal(self,directory, data_videos, n_frames, step_size):
        ...
        # Save files
        mode = '_val' if self.validation else '_train'
        with open(os.path.join(path,f'equal_{n_frames}{mode}_{step_size}_inputs.pkl'),'wb') as f:
            # pickle.dump(self.data_videos,f)
            pickle.dump(data_videos,f)

    def load_equal(self,directory, n_frames, step_size):
        mode = '_val' if self.validation else '_train'
        with open(os.path.join(directory,f'equal_{n_frames}{mode}_{step_size}_inputs.pkl'),'rb') as f:
        # with open(os.path.join(directory,'processed',f'standardized_processed{mode}_inputs.pkl'),'rb') as f:
            # self.inputs = pickle.load(f)
            # self.data_videos = pickle.load(f)
            equal_data_videos = pickle.load(f)
    ...
    def __getitem__(self, i):
        if self.mode == 'recon':
            ...
            # print('data',self.data.shape, 'i', i)
            # frames = data[i]
